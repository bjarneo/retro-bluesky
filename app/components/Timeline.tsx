...
export default function Timeline({ data, feed }: { data: BlueskyFeedData; feed: any[] }) {
  ...
  return (
    <div
      className={`flex flex-col justify-center items-center min-h-screen ${darkMode ? 'bg-[#000033]' : 'bg-[#0055aa]'} p-4`}
    >
      {/* Logo Section */}
      <div className="mb-8">
      ...
      <div
        className={`w-full max-w-md ${darkMode ? 'bg-[#000066] text-[#ffffff]' : 'bg-[#fff]'} border-4 border-[#000] shadow-[8px_8px_0_#000]`}
      >
        <div
          className={`${darkMode ? 'bg-[#000033]' : 'bg-[#0055aa]'} text-white p-2 font-bold border-b-4 border-[#000] flex justify-between items-center`}
        >
        ...
        {showProfile && (
          <div
            className={`p-4 ${darkMode ? 'bg-[#000066] text-[#ffffff]' : 'bg-[#dedede]'} border-b-4 border-[#000]`}
          >
            <h2 className="retro-font text-lg mb-2">Creator of this page</h2>
            <p className="retro-font">
              <a
                href="https://dothash.win"
        ...
        <div className="p-4">
          ...
          {feed.map((post) => (
            <div
              key={post.post.uri}
              className="border-2 border-[#000] shadow-[4px_4px_0_#000] p-4 mb-4"
            >
              ...
              <p className="mb-2 break-words whitespace-pre-wrap">{post.post.record.text}</p>
            </div>
          ))}
        </div>
      </div>
